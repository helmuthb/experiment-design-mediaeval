...
We had all the fucking code for subtask 2 but we did not manage to execute it and we did not manage to ask for another fucking extension of the deadline. WTF?
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item save all track activations for each dataset
    \item use the track activations from each dataset as input to train a new model
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item We then predict allmusic dataset on all 4 models and save the activations
  \item We the concatenate the 4 activations to one input to a new model which is trained on allmusic again
\end{itemize}

For the ROC/AUC we first had an implementation which worked with pytorch 1.4 and macosx, but with pytorch 1.1 and docker/linux we faced the problem of 0 values for this metrics. Fixing this now broke the ROC/AUC metrics on pytorch 1.4 and macosx. Since we used linux for the complete dataset we did not further investigate this curious difference.

We did not find a working python environment with gpu utilization for tartarus project, so we decided to implement the steps for the subtasks with pytorch with gpu support ourselves.

We compared the number of trainable parameters of our implementation of the networks with the number in the tartarus example and ensured that the numbers are equal.

The models for subtask 1 can be created by executing run\_subtask1.sh, the activations by executing prepare\_subtask2.sh and the final subtask 2 models by run\_subtask2.sh.

The activations that we created with prepare\_subtask2.py produced output which we were able to feed into the tartarus code and into our train-subtask2.py.
